...
export default function LoginForm() {
  ...
  const handleLogin = async (ev) => {
    ...
    try {
      ...
      const resp = await axios.post(`${API_BASE}/auth/login`, payload, { timeout: 10000 })
  ...
  const handleForgot = async (ev) => {
    ...
    try {
      const resp = await axios.post(`${API_BASE}/auth/forgot`, { email: forgotEmail }, { timeout: 10000 })
      if (resp && resp.data) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-900 p-6" dir="ltr">
        ...
        <div className="bg-white p-8 md:p-12 flex items-center">
          <div className="w-full">
            ...
            <div className="mt-6 text-xs text-gray-400 text-center">
              Need help? contact info@rockbridge.store
            </div>
          </div>
        </div>
      </div>
